...
@Component({
  ...
  template: `
    ...
    >
      <canvas
        #canvas
        class="hii-high__canvas"
        role="img"
        [attr.aria-label]="descriptor().accessibleLabel"
      ></canvas>

      <div
        class="hii-high__optics"
        aria-hidden="true"
      ></div>
    </div>
  `,
...
export class HiiRegionHighRender
  implements
    AfterViewInit,
    OnDestroy {
  ...
  /* glsl */ `
    ...
    float sourceField(
      ...
      if (uSourceCount > 7.5) {
        ...
        field += uSource7.z * exp(-d7 * d7 * 12.0);
      }
    ...
    float cavityField(
      ...
      if (uSourceCount > 7.5) {
        float d7 = sourceDistance(point, uSource7);
        cavity = max(cavity, 1.0 - smoothstep(0.07, 0.24, d7));
      }
    ...
    void main() {
      ...
      for (
        ...
        /*
         * HIGH V1: inherit the frozen LOW/MODERATE morphology grammar, but
         * push it into a denser multi-source regime with overlapping cavities,
         * stronger fronts, brighter filaments and more embedded ionizing stars.
         */
