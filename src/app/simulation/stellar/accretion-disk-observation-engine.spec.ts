...
describe('28.1 — existing active disks and observed provenance', () => {
  for (const version of [GeneratorVersion.V1, GeneratorVersion.V2]) {
    ...
    it(`uses the same active nucleus and disk in generator version ${version.code}, only after CONFIRMED`, () => {
      const existing = CompactAccretionEngine.fromExistingGalaxy(GalaxyGenerator.generate(key, 20n));
      expect(existing).not.toBeNull();
      expect(AccretionDiskObservationEngine.existingDiskOrNull(key, 20n, DiscoveryState.DETECTED)).toBeNull();
      expect(AccretionDiskObservationEngine.existingDiskOrNull(key, 20n, DiscoveryState.DISCOVERED)).toBeNull();
      expect(AccretionDiskObservationEngine.existingDiskOrNull(key, 20n, DiscoveryState.CATALOGUED)).toBeNull();
      expect(AccretionDiskObservationEngine.existingDiskOrNull(key, 20n, DiscoveryState.CONFIRMED))
        .toEqual(existing?.disk);
      expect(existing?.jet).toBeNull();
    });
    it(`never makes a disk for a quiet nucleus in version ${version.code}`, () => {
      expect(CompactAccretionEngine.fromExistingGalaxy(GalaxyGenerator.generate(key, 0n))).toBeNull();
      expect(AccretionDiskObservationEngine.existingDiskOrNull(key, 0n, DiscoveryState.CONFIRMED)).toBeNull();
    });
  }
