...
import { type GalaxyKnownWaterMoonIndex } from '../../domain/exploration/galaxy-known-water-moon-index';
...
import { GalaxyKnownWaterMoonIndexEngine } from '../../simulation/exploration/galaxy-known-water-moon-index-engine';
...
export interface GalaxyWaterMoonIndexModel {
  ...
  readonly index: GalaxyKnownWaterMoonIndex;
}

export type GalaxyWaterMoonIndexUiState =
  | Readonly<{ kind: 'loading' }>
  | Readonly<{ kind: 'empty' }>
  | Readonly<{ kind: 'not-found' }>
  | Readonly<{ kind: 'error'; message: string }>
  | Readonly<{ kind: 'content'; model: GalaxyWaterMoonIndexModel }>;

@Injectable({ providedIn: 'root' })
export class GalaxyWaterMoonIndexFacade {
  ...
  private readonly stateSignal = signal<GalaxyWaterMoonIndexUiState>({ kind: 'loading' });
    ...
    try {
      ...
      this.stateSignal.set({
        ...
        message: error instanceof Error && error.message.trim().length > 0
          ? error.message
          : 'No se pudo construir el índice galáctico de lunas con agua.',
      });
    }
  }
}
