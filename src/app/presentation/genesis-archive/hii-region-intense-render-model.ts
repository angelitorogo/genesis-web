...
export type HiiRegionIntenseMorphologyFamily =
  | 'BUBBLE'
  | 'BLISTER'
  | 'CLUMPY'
  | 'COMPACT'
...
export type HiiRegionIntensePaletteFamily =
  | 'ROSE_CYAN'
  | 'AMBER_TEAL'
  | 'VIOLET_BLUE'
  | 'MINT_GOLD'
...
export interface HiiRegionIntenseIonizingSource {
  readonly x:
    number;
...
export interface HiiRegionIntenseRenderModel {
  ...
  readonly ionizingSources:
    readonly HiiRegionIntenseIonizingSource[];
...
/**
 * Renderer-only V1 model for INTENSE-activity H II regions, derived from the frozen HIGH visual grammar.
 *
 * Goals of INTENSE V1:
 *
 * - preserve the same seed-fixed object across SIGNAL/IDENTIFIED/CATALOGUED/
 *   CONFIRMED;
 * - keep INTENSE activity clearly above HIGH with 8..12 dominant ionizing
 *   sources, broader source spacing, strongly overlapping cavities and several
 *   simultaneous excitation fronts;
 ...
 * - use the same palette families at their strongest plausible H II response,
 *   with a larger local dynamic range while retaining gas/dust readability;
 * - keep all visible structure renderer-only before scientific confirmation.
 */
export class HiiRegionIntenseRenderModelBuilder {
  ...
  static build(
    ...
    const structure =
      hashWords(
        `${descriptor.seed}/HII-INTENSE-STRUCTURE-V1`,
      );

    const volume =
      hashWords(
        `${descriptor.seed}/HII-INTENSE-VOLUME-V1`,
      );

    const morphology =
      hashWords(
        `${descriptor.seed}/HII-INTENSE-MORPHOLOGY-V1`,
      );

    const palette =
      hashWords(
        `${descriptor.seed}/HII-INTENSE-PALETTE-V1`,
      );

    const sources =
      hashWords(
        `${descriptor.seed}/HII-INTENSE-SOURCES-V1`,
      );
...
function intenseMorphologyProfileFor(
  base:
    MorphologyProfile,
): MorphologyProfile {
...
function intensePaletteProfileFor(
  base:
    PaletteProfile,
): PaletteProfile {
...
function buildIonizingSources(
  ...
  const result:
    HiiRegionIntenseIonizingSource[] =
    [];
  ...
  for (
    ...
    const words =
      hashWords(
        `${seed}/HII-INTENSE-IONIZER-${index}-V1`,
      );
...
function requireSupportedDescriptor(
  ...
  const hasIntenseProfile =
    descriptor.renderProfile ===
    ArchiveGalacticObjectRenderProfile
      .HII_INTENSE_VOLUME;

  const isConfirmedIntense =
    descriptor.kind ===
      ArchiveGalacticObjectRenderKind
        .HII_REGION &&
    descriptor.variant ===
  ...
  if (
    ...
    throw new RangeError(
      'HiiRegionIntenseRenderModelBuilder requires the opaque HII_INTENSE_VOLUME profile or a confirmed INTENSE H II descriptor.',
    );
  }
  ...
  if (
    ...
    throw new RangeError(
      'HiiRegionIntenseRenderModelBuilder only renders NEBULA/HII_REGION observational descriptors.',
    );
  }
  ...
  if (
    ...
    throw new RangeError(
      `HiiRegionIntenseRenderModelBuilder does not render variant ${descriptor.variant}.`,
    );
  }
