...
export type HiiRegionLowMorphologyFamily =
  | 'BUBBLE'
  | 'BLISTER'
  | 'CLUMPY'
  | 'COMPACT'
...
export type HiiRegionLowPaletteFamily =
  | 'ROSE_CYAN'
  | 'AMBER_TEAL'
  | 'VIOLET_BLUE'
  | 'MINT_GOLD'
...
export interface HiiRegionLowIonizingSource {
  readonly x:
    number;
...
export interface HiiRegionLowRenderModel {
  ...
  readonly ionizingSources:
    readonly HiiRegionLowIonizingSource[];
...
/**
 * Renderer-only V2 model for LOW-activity H II regions.
 *
 * Goals of the V2 refinement:
 *
 * - preserve the same seed-fixed object across SIGNAL/IDENTIFIED/CATALOGUED/
 *   CONFIRMED;
 * - keep LOW activity visually restrained (1..3 dominant ionizing sources);
 * - introduce much stronger morphology diversity without rewriting the full
 *   renderer from scratch;
 * - introduce stronger palette diversity so the laboratory A..H samples do not
 *   collapse into the same pink oval silhouette;
 * - keep all visible structure renderer-only before scientific confirmation.
 */
export class HiiRegionLowRenderModelBuilder {
  ...
  static build(
    ...
    const structure =
      hashWords(
        `${descriptor.seed}/HII-LOW-STRUCTURE-V2`,
      );

    const volume =
      hashWords(
        `${descriptor.seed}/HII-LOW-VOLUME-V2`,
      );

    const morphology =
      hashWords(
        `${descriptor.seed}/HII-LOW-MORPHOLOGY-V2`,
      );

    const palette =
      hashWords(
        `${descriptor.seed}/HII-LOW-PALETTE-V2`,
      );

    const sources =
      hashWords(
        `${descriptor.seed}/HII-LOW-SOURCES-V2`,
      );
...
function requireSupportedDescriptor(
  ...
  if (
    ...
    throw new RangeError(
      'HiiRegionLowRenderModelBuilder requires the opaque HII_LOW_VOLUME profile or a confirmed LOW H II descriptor.',
    );
  }
  ...
  if (
    ...
    throw new RangeError(
      'HiiRegionLowRenderModelBuilder only renders NEBULA/HII_REGION observational descriptors.',
    );
  }
  ...
  if (
    ...
    throw new RangeError(
      `HiiRegionLowRenderModelBuilder does not render variant ${descriptor.variant}.`,
    );
  }
